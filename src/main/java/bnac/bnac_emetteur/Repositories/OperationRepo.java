...
public interface OperationRepo extends JpaRepository<Operation,Integer> {
    ...
    @Query("Select new bnac.bnac_emetteur.DTO.MouvementsDTO(" +
            "tc.LibelleCourt, " +
            "COALESCE(cast(o.DateBourse as date), CURRENT_DATE), " +  // Ensure no null Date
            "COALESCE(a.RaisonSociale, ''), " +  // Ensure no null value for actionnaire
            "COALESCE(o.NumContrat, ''), " +     // Ensure no null value for numContrat
            "COALESCE(Cast((CASE WHEN o.typeOperation.IdTypeOperation = 'VC' THEN o.Quantite ELSE 0 END) AS integer), 0), " +  // Ensure no null for vente
            "COALESCE(Cast((CASE WHEN o.typeOperation.IdTypeOperation = 'AC' THEN o.Quantite ELSE 0 END) AS integer), 0), " +  // Ensure no null for achat
            "COALESCE(round(o.Cours, 3), 0.0f)) " +  // Ensure no null for cours
    ...
    @Query("Select new bnac.bnac_emetteur.DTO.MouvementsDTO(" +
            "tc.LibelleCourt, " +
            "COALESCE(cast(o.DateBourse as date), CURRENT_DATE), " + // Ensure no null Date
            "COALESCE(o.IdOperation, 0), " + // Ensure no null value for idOperation
            "COALESCE(o.NumContrat, ''), " + // Ensure no null value for numContrat
            "COALESCE(Cast((CASE WHEN o.typeOperation.IdTypeOperation = 'VC' THEN o.Quantite ELSE 0 END) AS integer), 0), " + // Ensure no null for vente
            "COALESCE(Cast((CASE WHEN o.typeOperation.IdTypeOperation = 'AC' THEN o.Quantite ELSE 0 END) AS integer), 0), " + // Ensure no null for achat
            "COALESCE(o.Cours, 0.0f)) " + // Ensure no null for cours
            "from Actionnaire a, Titre t, Operation o, TeneurCompte tc " +
            "Where a.Matricule = o.actionnaire.Matricule " +
            "and t.IdTitre = o.titre.IdTitre " +
            "and t.emetteur.idEmetteur = a.emetteur.idEmetteur " +
